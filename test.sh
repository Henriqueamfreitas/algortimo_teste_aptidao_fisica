...
while read -r test_case; do
  ...
  if [ "$output" = "$expected_output" ]; then
    ...
    echo -e "$x- \033[32mPASS\033[0m: $gender $height $barReps $barSeconds $abs $runDistance $runTime $swimDistance $swimTime $diveTime"
  else
    # Output result in red if the test failed
    echo -e "$x- \033[31mFAIL\033[0m: $gender $height $barReps $barSeconds $abs $runDistance $runTime $swimDistance $swimTime $diveTime"
  fi
  x=$(( $x + 1 ))
done <<< "$test_cases"
